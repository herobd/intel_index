...
(T * outputImage,
 ...
 int flags)
{
  T * tempImage = (T*) vl_malloc(sizeof(T) * numRows * numOutputColumns) ;
  vl_uindex k ;
  vl_index j ;
...
VL_EXPORT void
VL_XCAT(vl_matlab_smooth_, SFX)(const T* in, int height, int width, double sigma, T* out, int* outH, int* outW)
{
  ...
  ndims = 2;//mxGetNumberOfDimensions(IN(I)) ;
  //dims = mxGetDimensions(IN(I)) ;
  M = height;//dims[0] ;
  N = width;//dims[1] ;
  K = 1;//(ndims > 2) ? dims[2] : 1 ;
