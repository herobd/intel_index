function extract_FV_features_fast_subword(opts)

GMM = readGMM(opts.fileGMM);
PCA = readPCA(opts.filePCA);
imagesTOC = readImagesToc(opts.fileImages_subword);
...
% Write output header
fid = fopen(opts.fileFeatures_subword, 'w');
...
for cb=1:nBatches    
    ...
    % Read image batch
    [fid,msg] = fopen(opts.fileImages_subword, 'r');
    ...
    % Write the batch
    fid = fopen(opts.fileFeatures_subword, 'r+');    
