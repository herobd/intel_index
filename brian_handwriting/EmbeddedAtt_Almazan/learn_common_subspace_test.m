function [embedding,mAP,bestmAPval,data] = learn_common_subspace_test(opts,data)

%% Load attribute representations
attReprTr = data.test_attReprTr %readMat(opts.fileAttRepresTr);
attReprVa = data.test_attReprVa %readMat(opts.fileAttRepresVal);
...
%if opts.TestCCA
%    [embedding.cca,mAP.cca] = learnCCA_test(opts, data);
...
if opts.TestKCCA
    [embedding.kcca,mAP.kcca] = learnKCCA_test(opts, data);
