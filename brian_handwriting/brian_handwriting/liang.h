...
private:
    ...
    vector<list<const Grapheme*> > learnCharacterSegmentation(string query, vector<Grapheme*> graphemes, const Mat &skel, const list<Point>& localMaxs, const list<Point>& localMins, map<char, list<const Grapheme*> >* charGraphemes=NULL, map<char, list<int> >* accumWidths=NULL, map<char, int>* charCounts=NULL );
    void thinning(const cv::Mat& src, cv::Mat& dst);
    Mat breakSegments(const Mat& skel, list<Point>* minima=NULL, list<Point>* maxima=NULL);
    ...
    int xDelta(int direction);
    int yDelta(int direction);
//    void onLocals(Point &toAdd, list<Point>& after, list<Point>& before, Point& inQuestion, int neighborhoodSize, list<Point>& minima, list<Point>& maxima);
    ...
    void findBaselines(bool hasAscender, bool hasDescender, int* upperBaseline, int* lowerBaseline, const list<Point>& localMaxs, const list<Point>& localMins);
    ...
    void relabelStrech(int label, Point from, Mat& ret, MinMaxTracker& tracker);
